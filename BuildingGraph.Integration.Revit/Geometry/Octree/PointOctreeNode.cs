...
namespace HLApps.Revit.Geometry.Octree
{
    ...
    public class PointOctreeNode<T> where T : class
    {
        ...
        public PointOctreeNode(float baseLengthVal, float minSizeVal, XYZ centerVal)
        {
        ...
        /// <returns></returns>
        public bool Add(T obj, XYZ objPos)
        {
        ...
        */
        ...
        public PointOctreeNode<T> ShrinkIfPossible(float minLength)
        {
            ...
            if (children == null)
            {
                ...
                SetValues(SideLength / 2, minSize, childBounds[bestFit].MidPoint);
                return this;
            }
        ...
        */
        ...
        void Split()
        {
            ...
            children[0] = new PointOctreeNode<T>(newLength, minSize, Center + new XYZ(-quarter, quarter, -quarter));
            children[1] = new PointOctreeNode<T>(newLength, minSize, Center + new XYZ(quarter, quarter, -quarter));
            children[2] = new PointOctreeNode<T>(newLength, minSize, Center + new XYZ(-quarter, quarter, quarter));
            children[3] = new PointOctreeNode<T>(newLength, minSize, Center + new XYZ(quarter, quarter, quarter));
            children[4] = new PointOctreeNode<T>(newLength, minSize, Center + new XYZ(-quarter, -quarter, -quarter));
            children[5] = new PointOctreeNode<T>(newLength, minSize, Center + new XYZ(quarter, -quarter, -quarter));
            children[6] = new PointOctreeNode<T>(newLength, minSize, Center + new XYZ(-quarter, -quarter, quarter));
            children[7] = new PointOctreeNode<T>(newLength, minSize, Center + new XYZ(quarter, -quarter, quarter));
        }
        ...
        public static double DistanceToRay(Line ray, XYZ point)
        {
            //revit way of doing this:
            var pjPoint = ray.Project(point);
            return pjPoint.Distance;

            //orginal way of doing this
            //return (point - ray.Origin).CrossProduct(ray.Direction).GetLength();
        }
