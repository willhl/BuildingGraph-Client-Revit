...
namespace HLApps.Revit.Geometry.Octree
{
    ...
    public class BoundsOctreeNode<T>
    {
        ...
        {
            public T Obj;
            public HLBoundingBoxXYZ Bounds;
        }
        ...
        public BoundsOctreeNode(float baseLengthVal, float minSizeVal, float loosenessVal, XYZ centerVal)
        {
        ...
        /// <param name="result">List result.</param>
        /// <returns>Objects that intersect with the specified bounds.</returns>
        public void GetColliding(ref Line checkBounds, List<T> result, Filter<T> flt)
        {
            // Are the input bounds at least partially in this node?
            if (!bounds.IntersectRay(checkBounds))
            {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                if (objects[i].Bounds.IntersectRay(checkBounds))
                {
                    result.Add(objects[i].Obj);
                }
            }
        ...
        public void DrawAllBounds(float depth = 0)
        {
            ...
            Bounds thisBounds = new HLBoundingBoxXYZ(Center, new Vector3(adjLength, adjLength, adjLength));
        ...
        */
        ...
        public BoundsOctreeNode<T> ShrinkIfPossible(float minLength)
        {
            ...
            if (children == null)
            {
                ...
                SetValues(BaseLength / 2, minSize, looseness, childBounds[bestFit].MidPoint);
                return this;
            }
        ...
        */
        ...
        void Split()
        {
            ...
            children[0] = new BoundsOctreeNode<T>(newLength, minSize, looseness, Center + new XYZ(-quarter, quarter, -quarter));
            children[1] = new BoundsOctreeNode<T>(newLength, minSize, looseness, Center + new XYZ(quarter, quarter, -quarter));
            children[2] = new BoundsOctreeNode<T>(newLength, minSize, looseness, Center + new XYZ(-quarter, quarter, quarter));
            children[3] = new BoundsOctreeNode<T>(newLength, minSize, looseness, Center + new XYZ(quarter, quarter, quarter));
            children[4] = new BoundsOctreeNode<T>(newLength, minSize, looseness, Center + new XYZ(-quarter, -quarter, -quarter));
            children[5] = new BoundsOctreeNode<T>(newLength, minSize, looseness, Center + new XYZ(quarter, -quarter, -quarter));
            children[6] = new BoundsOctreeNode<T>(newLength, minSize, looseness, Center + new XYZ(-quarter, -quarter, quarter));
            children[7] = new BoundsOctreeNode<T>(newLength, minSize, looseness, Center + new XYZ(quarter, -quarter, quarter));
        }
