...
namespace HLApps.Revit.Geometry.Octree
{
    ...
    public class BoundsOctree<T>
    {
        ...
        public BoundsOctree(float initialWorldSize, XYZ initialWorldPos, float minNodeSize, float loosenessVal)
        {
            ...
            minSize = minNodeSize;
            looseness = 1.0f; //Mathf.Clamp(loosenessVal, 1.0f, 2.0f);
        ...
        /// <param name="checkBounds">bounds to check.</param>
        /// <returns>Objects that intersect with the specified bounds.</returns>
        public T[] GetColliding(Line ray, Filter<T> flt = null)
        {
            //#if UNITY_EDITOR
            // For debugging
            //AddCollisionCheck(checkBounds);
            //#endif
            List<T> collidingWith = new List<T>();
            rootNode.GetColliding(ref ray, collidingWith, flt);
            return collidingWith.ToArray();
        }
